...
async def send_msg(result, matcher, event):
    ...
    if is_successful:
        ...
        if is_suggest_able == 'True':
            ...
            msg = f"{last_answer}"
        #logger.info(f"plain_text: {msg}")
        if "Dev Mode" in msg:
             logger.info(f"plain_text: {msg}")
             Dev_reply = re.search(r"Dev Mode: ", msg)
             Dev_reply_textstart = Dev_reply.start()
             logger.info(f"Dev_text: {Dev_reply_textstart}")
             msgid_container = await send_message(msg[Dev_reply_textstart+9:])
